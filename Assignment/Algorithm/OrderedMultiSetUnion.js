...
function orderedMultisetUnion(sortedA, sortedB) {
    ...
    while (i < sortedA.length && j < sortedB.length){
        ...
        if (sortedA[i] === sortedB[j]){
            ...
            (sortedA[i] < sortedB[j]) ? result.push(sortedA[i++]) : result.push(sortedB[j++]);
        }
    }

    (i < sortedA.length) ? result.push(...sortedA.slice(i)) : null;
    
    (j < sortedB.length) ? result.push(...sortedB.slice(j)) : null;
